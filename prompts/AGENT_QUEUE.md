...
# §PAGES-SERVES-THE-BRANCH 2026-09-04 (last) — **READ THIS FIRST ON RESTART.**
**The single most load-bearing fact learned this session, and it invalidates a pattern three things in
this repo rely on.**

## §PZ.1 THE FACT, measured
```
gh api repos/red1oon/bim-ootb/pages
{"build_type":"legacy","source":{"branch":"main","path":"/"}}
```
**GitHub Pages for `bim-ootb` publishes the TRACKED FILES ON `main`.** The artifact
`.github/workflows/deploy-pages.yml` builds and uploads is **served by nothing.** The workflow runs,
goes green, and its output reaches no user.

Measured on `https://red1oon.github.io/bim-ootb`, the same minute:

| path | live | what it proves |
|---|---|---|
| `erp/sw.js` | **200**, `CACHE_VERSION v792` — identical to `main` | the site is CURRENT, not stale. The 404s below are not a deploy lag |
| `erp/version.json` | **404** | generated by that workflow's own first step — **404 since it was introduced** |
| `erp/erp_picker.js` | **200, 36,897 bytes — byte-identical to the repo source** | the minify step has NEVER reached a user |
| `erp/*_agent.zip` (while untracked) | **404** | my own regression, below |

**Three things believed live are not:** `erp/version.json` (the System Monitor's Release row reads it),
`UI_PAYLOAD_PERF.md` Win #1's minification (claimed **gz 1167 KB → 737 KB, −36.9%**), and — for about
three hours — both shipped agent downloads.

## §PZ.2 THE REGRESSION I SHIPPED, and how it was caught
`§AGENT-ZIPS-BUILT` (#1675) untracked `erp/*_agent.zip` and had `deploy-pages.yml` build them,
**citing `erp/version.json` as the precedent for "generated into the artifact, never committed".** The
precedent was itself broken. Both downloads 404'd live.

**It was caught only by fetching the live URL.** The PR was green, CI was green, the deploy workflow
was green, `W-AGENT-ZIP-SYNC` was 13/0 — and the feature was dead. Fixed in **#1680**: the zips are
tracked again; the builder, the restored `extract_model.js` and the `§AZ.3` nesting all stay, and
`W-AGENT-ZIP-SYNC` gained **A0a** (*"the zip is PRESENT"*) so untracking it again fails loudly instead
of silently 404ing.

**VERIFIED LIVE after #1680**, not inferred: both `200 application/x-zip-compressed`
(9,808 / 22,706 bytes); extracting both into one empty directory yields exactly `idempiere_agent/` and
`odoo_agent/` with nothing loose; `idempiere_agent/migrate_agent.js` and `odoo_agent/agent.js` and
`odoo_agent/extract_model.js` all resolve — the three paths `common/about_diy.js` tells users to run —
and `diff -r` against both source directories is clean.

## §PZ.3 ⛔ USER — switch Pages to `build_type: "workflow"`?
It would make `version.json` and the **minification live for the first time** (−36.9% gz on the whole
app), and make the site **stop updating whenever the workflow fails** — today a bad workflow is
invisible because nothing depends on it. Real blast radius both ways; a repo-settings change, not
work. **Until it is taken, the rule is: anything a user must be able to FETCH has to be COMMITTED.**

## §PZ.4 THE STANDING RULE THIS EARNS
**A deploy is not verified by a green workflow. Fetch the URL.** This session shipped four PRs whose
witnesses were green and whose CI was green; the fifth thing — a live `curl` — is what found a dead
feature. `feedback_verify_the_end_of_the_chain.md` already says *uploaded ≠ live*; this is the same
rule with a new mechanism attached, and the mechanism is the part worth remembering: **on this repo the
deploy pipeline and the thing being served are not connected.**

---

# §RESTART 2026-09-04 — where this lane stands, in one block
**`§ERP-SESSION-CLOSE-2 §C2.3` is DRAINED — all five ranked items closed, plus the two follow-ups the
user asked for.** Six PRs, all merged, all CI-green: bim-ootb **#1669 · #1672 · #1673 · #1675 · #1677
· #1680**. sw `v790 → v792`.

| what | measured before → after | section |
|---|---|---|
| E-1 callout campaign | live dispatch **3 → 51 of 78** bindings on the nine document tables | `§CALLOUT-CAMPAIGN` |
| form #2 of 49 | AD_Form 103 `VTrxMaterial`; **1 → 2 of 49** forms | `§ADFORM-TRXMATERIAL` |
| 4 stale live witnesses | all RED → all GREEN; **none a product defect** | `§STALE-WITNESSES` |
| 2 tracked binaries | `preview_demo.db` built from its `.dump`; zips built + drift-checked | `§PREVIEW-DEMO-FROM-SQL` · `§AGENT-ZIPS-BUILT` |
| `§AZ.3` answered | `idempiere_agent.zip` nests — the `cd idempiere_agent` the app prints now runs | `§AZ.3` |
| the Pages fact | both downloads 404'd live, re-tracked and **verified by `curl`** | `§PAGES-SERVES-THE-BRANCH` |

**Four new witnesses, each RED on plain `origin/main` first:** `W-CALLOUT-CAMPAIGN` 29/0 (was 9/20) ·
`W-ADFORM-TRXMATERIAL` 19/0 (was INCONCLUSIVE) · `W-AGENT-ZIP-SYNC` 15/0 (was 1 FAIL) · plus the four
repaired live instruments.

**Four defects nobody had asked for, every one found by RUNNING something:**
1. **`AdCallout.installDefaultHandlers()` was never called by anything** — six engine line callouts had
   never fired in a browser. `§IC.3`'s "28 dispatch = 36%" counted a paper registry; live was **4%**.
2. **`odoo_agent.zip` shipped without `odoo_agent/extract_model.js`** — the file its own extraction step
   needs, in a bundle advertised as self-sufficient.
3. **`common/about_diy.js` printed `cd idempiere_agent` for a zip with no such folder** — the command
   the app tells users to run could not run.
4. **The Pages deploy artifact is published by nothing** (`§PZ.1`) — three "live" things are not.

## ⬜ NEXT, ranked, nothing blocked
1. **`§SW.2` — 12 more witnesses default to a `/tmp/wt-*` worktree that is measured GONE.** Nine take
   the same one-line default; each needs one run to confirm it still passes against the live checkout.
   Three declare their pin deliberate and are a judgement call, not a typo fix. **Listed in full there.**
2. **`§CC.7` — 27 callout bindings still undispatched on the nine tables.** Ranked there. Note four are
   `navigate*` atoms that move the CURSOR, not derives: they should leave the denominator, not be
   ported. **`CalloutOrder.docType` stays out** — second writer to `IsSOTrx` (`§CC.5`).
3. **`§AF.5` — 47 of 49 forms have no renderer.** A new one is one `_registerForm` call; pick the read
   face of something already written, the test `§TM.1` used.
4. **`§PZ.3` ⛔USER** — switch Pages to `build_type: "workflow"`?

## The two habits this session paid for repeatedly
- **Check the instrument before believing the defect.** Three of four red witnesses named the wrong
  thing; a coverage number read 36% on paper and 4% in the page; a backlog's "37-binding gap" was 50;
