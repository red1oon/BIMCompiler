...
- **Datums 802 → 657 on SampleCastle.** Plausibly correct — anchor-displaced faces were manufacturing
  false alignment clusters — but UNVERIFIED. It bears directly on **row 7**, the grid-lock crux and its
  0.104 m emergent-grid residual, because that grid is built from these datums. **Re-measure row 7 before
  anyone spends the "heavy session" it asks for** — its baseline may have moved under it.
