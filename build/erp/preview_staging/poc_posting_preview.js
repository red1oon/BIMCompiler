...
var AD = path.join(ERP, 'ad_seed.db');                         // gate source (ad_role 102=Y/103=N)
var GLASS = path.join(__dirname, 'fixtures', 'glassbowl_data.db'); // derive source (full FOLD oracle db)
...
(async function () {
  ...
  var adDb = new SQL.Database(new Uint8Array(fs.readFileSync(AD)));
  var glassDb = new SQL.Database(new Uint8Array(fs.readFileSync(GLASS)));
  var adQ = function (sql, p) { return rows(adDb, sql, p); };
  var docDb = PV.facade(glassDb);                              // the browser sql.js handle, better-sqlite3-shaped
  ...
  function oracleInvoice(invId) {
    var r = rows(glassDb, 'SELECT account_id, ROUND(SUM(amtacctdr),2) dr, ROUND(SUM(amtacctcr),2) cr FROM fact_acct WHERE ad_table_id=? AND record_id=? AND c_acctschema_id=? GROUP BY account_id', [AD_TABLE_C_INVOICE, invId, SCHEMA]);
  ...
  console.log('\n── §PREVIEW-NOWRITE ──');
  var before = glassDb.export();
  PV.previewDocAction(refSO, ADMIN_CTX, dbs, opts);
  var after = glassDb.export();
